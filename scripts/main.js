import {
  CURSES_DAMNATIONS_CONTENT_VERSION,
  CURSES_DAMNATIONS_DEFINITIONS,
  CURSES_DAMNATIONS_MODULE_ID
} from "./content.js";

const MODULE_ID = CURSES_DAMNATIONS_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "curses-damnations";
const LIBRARY_ID = "curses-damnations.core";
const PACK_NAME = "affliction-forge-curses-damnations";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_CD.Module.Library", "Curses & Damnations"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: CURSES_DAMNATIONS_CONTENT_VERSION,
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_CD.Module.Title", "PF2E Affliction Forge: Curses & Damnations"),
    moduleId: MODULE_ID,
    version: CURSES_DAMNATIONS_CONTENT_VERSION,
    metadata: {
      themes: ["curse", "damnation", "hex"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_CD.Module.LibraryDescription", "Original curses and damnations with semantic Creature Forge tags."),
      metadata: {
        themes: ["curse", "damnation", "hex"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: CURSES_DAMNATIONS_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: CURSES_DAMNATIONS_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_CD.Module.SyncFailed", "Curses & Damnations could not synchronize its library. See console.")
    );
  });
});
